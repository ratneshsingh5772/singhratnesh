...
const Portfolio = () => {
  ...
  return (
    <div className="container mx-auto px-4" style={{ background: "linear-gradient(135deg, #b3c6ff, #c2d1ff)", padding: "3rem 1rem", minHeight: "100vh" }}>
      <div className="rounded-xl" style={{ boxShadow: "0 10px 40px rgba(0,0,0,0.3)", borderRadius: "20px", overflow: "hidden", border: "none" }}>
        ...
        <section className="p-4 text-center">
          {categories.map((category) => (
            <button 
              key={category}
              className={`mx-2 px-6 py-3 rounded-full font-bold transition-all ${selectedCategory === category ? "bg-blue-500 text-white" : "border-2 border-blue-500 text-blue-500 hover:bg-blue-500 hover:text-white"}`}
        ...
        <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 p-4">
          {filteredItems.map((item) => (
            <div key={item.id} className="">
              ...
              >
                <div className="p-6" style={{ background: "transparent", textAlign: "center" }}>
                  <h5 className="font-bold text-xl" style={{ fontWeight: "700", fontSize: "1.5rem", color: "#333" }}>{item.title}</h5>
                  <p className="text-base" style={{ fontSize: "1rem", color: "#555" }}>{item.description}</p>
                </div>
              </div>
            </div>
          ))}
        ...
        {/* Navigation Buttons */}
        <footer className="flex justify-between p-4" style={{ background: "#eef2ff" }}>
          <button className="border-2 border-blue-500 text-blue-500 px-5 py-2 rounded hover:bg-blue-500 hover:text-white transition-colors font-bold" onClick={handlePrev} style={{ fontWeight: "700", padding: "10px 20px" }}>
            <FaArrowLeft /> Prev
          </button>
          <button className="border-2 border-blue-500 text-blue-500 px-5 py-2 rounded hover:bg-blue-500 hover:text-white transition-colors font-bold" onClick={handleNext} style={{ fontWeight: "700", padding: "10px 20px" }}>
            Next <FaArrowRight />
          </button>
        </footer>
      </div>
